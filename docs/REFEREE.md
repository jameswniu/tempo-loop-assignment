...
```
Claim     "16 or 17 of 20 checks on each of eight consecutive runs, a median of 85%" (NOTES.md, README.md section 3, the README hero tile reading 85% eval pass, median of 8, and the eval badge)
Unit      individual assertions passed across the four frozen cases in one run of the suite, where the denominator moves with the run because a case that fails to return the agreed shape contributes one check instead of five
...
Set       four cases in evals/cases, frozen from honojs/hono, fastify/fastify, vitest-dev/vitest and jameswniu/multi-agent-rl-mapf-drone-navigation, all over 2026-06-01 to 2026-09-01, answered by qwen-plus through Alibaba's OpenAI-compatible endpoint at default temperature
Command   npm run eval -- openai, roughly 25 seconds, and eight consecutive runs produced 16, 17, 16, 16, 17, 17, 17 and 17. Two later runs produced 14 of 16, with one case returning nothing usable, and 18 of 20. Both are stated in NOTES.md and neither is in the sample the median is taken over. The sample is committed as evals/sample.json, tools/figures.ts generates the hero tile from it, and npm run figures:check fails if the README badge or prose stops matching it
Sibling   UNMEASURED against Claude or GPT. The shipped default is Anthropic and no run has ever gone through it, so this number describes one model that is not the one the README tells a reviewer to configure. The honest sibling within this model is the sample this replaced: five earlier runs read 17, 18, 18, 18, 18, and reporting that streak would have overstated the median by four points and hidden the two runs in eight that produced no usable answer at all
Bound     eight runs of a sampled model is a small sample of a stochastic process, the bands were written by the same person who wrote the prompt, and the failures are the same ones every time rather than scattered, which makes this closer to one repeated observation than to eight independent ones
Knob      the confidence bands in evals/cases.ts and the arithmetic rule in the system prompt, since both failures are the model computing a total it was told not to compute
```

```
Claim     "npm test runs 131 tests" (NOTES.md line 35, the README hero tile reading 131 tests, rules pinned, and the tests badge)
